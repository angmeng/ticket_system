class OrderingMachine
  ...
  def initialize(order, company, branch, params_payment_type)
    ...
    @order.amount_tender   = params_payment_type[:amount_tender]
    @order.remark          = params_payment_type[:remark]
    @order.branch_id       = @branch.id
    @order.total_passenger = @total_ticket
    @order.reservation     = true if @order_params["reservation"] == "yes"
  end
  ...
  def make_order
    ...
    create_buyerType_and_tripType
   	@order.save!
    generate_reservation_detail(@order) if @order_params["reservation"] == "yes"
  end
  ...
  def make_departure_order
    if @order_params["adult"].to_i > 0
      ...
      @item.status_id = OrderStatus::VERIFIED unless @order_params["reservation"] == "yes"
      if @item.save
        generate_item_detail(@item) unless @order_params["reservation"] == "yes"
  ...
  def generate_reservation_detail(order)
    @current_reservation_no = @company.last_reservation_number
    @reservation_detail = ReservationDetail.new(:reservation_number => "RSV #{"%05d" % @current_reservation_no}", :order_id => order.id)
    @reservation_detail.save!
    @current_reservation_no += 1
    @company.update_attributes!(:last_reservation_number => @current_reservation_no)
  end
  ...
  def recover_departure_balance(departure_id)
    dep = Departure.find(departure_id)
    dep.counter_sales -= @order.total_passenger
    dep.save!
  end
  ...
  def self.update_order_items_for_reservations(order, company)
    @company  = company
    @order    = order
    @items    = @order.order_items
    if @items.present?
      @items.each do |item| 
        item.update_attributes(:status_id => OrderStatus::VERIFIED)
        self.module_item_detail(item)
      end
    end
  end

  # Updating Reservation
  def self.module_item_detail(order_item)
    count_adult         = order_item.number_of_adult.to_i
    count_child         = order_item.number_of_kid.to_i
    count_infant        = order_item.number_of_infant.to_i
    @current_voucher_no = @company.last_voucher_number
    ...
    if count_adult > 0
      count_adult.times.each do |adult|
        @detail = order_item.order_item_details.new
        self.module_adding_detail(order_item)
        @detail.fare = order_item.adult_fare
        @detail.ticket_category_id = TicketType::ADULT
        @detail.save!
      end
  ...
  # Updating Reservation
  def self.module_adding_detail(order_item)
    if @order.is_open_ticket?
      if order_item.is_coming_back?
        @detail.ticket_status_id = TicketStatus::VOUCHER
        @detail.serial_number = "VOU #{"%05d" % @current_voucher_no}" 
  ...
  def self.store_order(order)
    ...
    @ret[:operate] = "yes"
    @order_params = order
    @ret[:reservation] = "yes" if @order_params["reservation"] == "yes"
    @item = OrderItem.new
